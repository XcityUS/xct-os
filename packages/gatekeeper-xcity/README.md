...
- **Context store:** the `XcityContext` resource reads and writes the user's cross-product context
  documents served by TokenHub's `/v1/xct-context` API (documents shared by all Xcity products).
  Reads are live; saves, updates, and deletes are staged through the approval queue and report
  their outcome via `getWrite()`. Documents are capped at 256 KB of UTF-8 content.
...
The Xcity Context resource does not need the media worker; it appears whenever
`XCITY_TOKENHUB_URL`, `XCITY_WALLET_URL`, and `WALLET_SERVICE_TOKEN` are all set (the wallet
settings mint the same per-user TokenHub virtual key the media resource uses). If any of the three
is missing, the context resource is hidden.
